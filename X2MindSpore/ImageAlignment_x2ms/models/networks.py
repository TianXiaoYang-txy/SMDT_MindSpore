...
def get_norm_layer(norm_type='instance'):
    ...
    """
    if norm_type == 'batch':
        norm_layer = functools.partial(x2ms_adapter.nn.BatchNorm2d, affine=True, track_running_stats=True)
...
def get_scheduler(optimizer, opt):
    ...
    if opt.lr_policy == 'linear':
        ...
        scheduler = lr_schedule_wrapper.LambdaLR(optimizer, lr_lambda=lambda_rule)
    ...
    def init_func(m):  # define the initialization function
        ...
        if hasattr(m, 'weight') and (classname.find('Conv') != -1 or classname.find('Linear') != -1):
            ...
            if hasattr(m, 'bias') and m.bias is not None:
                x2ms_adapter.nn_init.constant_(m.bias.data, 0.0)
        elif classname.find(
                'BatchNorm2d') != -1:  # BatchNorm Layer's weight is not a matrix; only normal distribution applies.
            x2ms_adapter.nn_init.normal_(m.weight.data, 1.0, init_gain)
            x2ms_adapter.nn_init.constant_(m.bias.data, 0.0)
...
def init_net(net, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    """
    if len(gpu_ids) > 0:
        assert (x2ms_adapter.is_cuda_available())
        net.to(gpu_ids[0])
...
class GANLoss(nn.Cell):
    ...
    def __init__(self, gan_mode, target_real_label=1.0, target_fake_label=0.0):
        ...
        self.register_buffer('real_label', x2ms_adapter.tensor(target_real_label))
        self.register_buffer('fake_label', x2ms_adapter.tensor(target_fake_label))
        self.gan_mode = gan_mode
        if gan_mode == 'lsgan':
            self.loss = nn.MSELoss()
        elif gan_mode == 'vanilla':
            self.loss = loss_wrapper.BCEWithLogitsLoss()
    ...
    def __call__(self, prediction, target_is_real):
        ...
        if self.gan_mode in ['lsgan', 'vanilla']:
            ...
            if target_is_real:
                loss = -x2ms_adapter.tensor_api.mean(prediction)
            else:
                loss = x2ms_adapter.tensor_api.mean(prediction)
        return loss
...
def cal_gradient_penalty(netD, real_data, fake_data, device, type='mixed', constant=1.0, lambda_gp=10.0):
    ...
    if lambda_gp > 0.0:
        if type == 'real':  # either use real images, fake images, or a linear interpolation of two.
            ...
            alpha = x2ms_adapter.rand(real_data.shape[0], 1, device=device)
            alpha = x2ms_adapter.tensor_api.expand(alpha, real_data.shape[0], real_data.nelement() // real_data.shape[0]).contiguous().view(
                *real_data.shape)
        ...
        gradient_penalty = x2ms_adapter.tensor_api.mean((((gradients + 1e-16).norm(2, dim=1) - constant) ** 2)) * lambda_gp  # added eps
...
class XFork_Shared_Encoder_Block(nn.Cell):
    def __init__(self, input_nc, norm_layer, use_dropout, use_bias):
        ...
        nc_output_tmp = 64
        conv = x2ms_adapter.nn.Conv2d(nc_input_tmp, nc_output_tmp, kernel_size=4, stride=2, padding=1, bias=use_bias)  # 128x128
        act = nn.LeakyReLU(negative_slope)
        generator_block += [conv, act]

        nc_input_tmp = nc_output_tmp
        nc_output_tmp = 128
        conv = x2ms_adapter.nn.Conv2d(nc_input_tmp, nc_output_tmp, kernel_size=4, stride=2, padding=1, bias=use_bias)  # 64x64
        ...
        nc_input_tmp = nc_output_tmp
        nc_output_tmp = 256
        conv = x2ms_adapter.nn.Conv2d(nc_input_tmp, nc_output_tmp, kernel_size=4, stride=2, padding=1, bias=use_bias)  # 32x32
        norm = norm_layer(nc_output_tmp)  # swap norm and act for fine-turning
        act = nn.LeakyReLU(negative_slope)
        generator_block += [conv, norm, act]
        ...
        for i in range(1, 5):  # 16x16 -> 8x8 -> 4x4 -> 2x2 ->1x1
            nc_input_tmp = nc_output_tmp
            nc_output_tmp = 512
            conv = x2ms_adapter.nn.Conv2d(nc_input_tmp, nc_output_tmp, kernel_size=4, stride=2, padding=1, bias=use_bias)
            norm = norm_layer(nc_output_tmp)  # swap norm and act for fine-turning
            act = nn.LeakyReLU(negative_slope)
            generator_block += [conv, norm, act]
        ...
        nc_output_tmp = 512
        conv = x2ms_adapter.nn.Conv2d(nc_input_tmp, nc_output_tmp, kernel_size=4, stride=2, padding=1, bias=use_bias)
        norm = x2ms_adapter.nn.BatchNorm2d(nc_output_tmp)
        act = x2ms_adapter.nn.ReLU(True)
        generator_block += [conv, norm, act]

        self.model = x2ms_adapter.nn.Sequential(*generator_block)
...
class XFork_Shared_Decoder_Block(nn.Cell):
    def __init__(self, input_nc, norm_layer, use_dropout, use_bias):
...
class XFork_Separate_Decoder_Block1(nn.Cell):
...
class XFork_Separate_Decoder_Block2(nn.Cell):
    def __init__(self, input_nc, output_nc, norm_layer, use_dropout, use_bias):
...
class ResnetGenerator(nn.Cell):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=x2ms_adapter.nn.BatchNorm2d, use_dropout=False, n_blocks=6,
        ...
        for i in range(n_downsampling):  # add downsampling layers
            mult = 2 ** i
            model += [x2ms_adapter.nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=2, padding=1, bias=use_bias),
        ...
        self.model = x2ms_adapter.nn.Sequential(*model)
...
#--------------------- UnetAFL_4b----------------------------------------
class UnetAFL_4b(nn.Cell):
    """Resnet-based generator that consists of Adversarial Feedback Loop. with Feedback Adversarial Learning - cvpr19
    """
    def __init__(self, input_nc, output_nc, ngf=64, ndf=64, norm_layer=x2ms_adapter.nn.BatchNorm2d, use_dropout=False,
    ...
    def _forward_afl(self, fea_list):
        ...
        feedback_input_new = x2ms_adapter.cat((self.netGA.feedback0, feedback_input), 1)
        ...
        feedback_input = x2ms_adapter.cat((feedback_input, fea_list[1]), 1)
        feedback_input_new = x2ms_adapter.cat((self.netGA.feedback1, feedback_input), 1)
        ...
        feedback_input = x2ms_adapter.cat((feedback_input, fea_list[2]), 1)
        feedback_input_new = x2ms_adapter.cat((self.netGA.feedback2, feedback_input), 1)
        ...
        feedback_input = x2ms_adapter.cat((feedback_input, fea_list[3]), 1)
        feedback_input_new = x2ms_adapter.cat((self.netGA.feedback3, feedback_input), 1)
        ...
        feedback_input = x2ms_adapter.cat((feedback_input, fea_list[4]), 1)
        feedback_input_new = x2ms_adapter.cat((self.netGA.feedback4, feedback_input), 1)
    ...
    def construct(self, x):
        ...
        fake_AB = x2ms_adapter.cat((x, gen_img),
        ...
        for step in range(1, self.loop_count):
            ...
            gen_img = gen_output[:, 0:3, ::]
            gen_seg = gen_output[:, 3:6, ::]
            fake_AB = x2ms_adapter.cat((x, gen_img),                                1)  # we use conditional GANs; we need to feed both input and output to the discriminator
...
class BasicUNetGenerator_4b(nn.Cell):
    """
    network structure of Feedback Adversarial Learning - cvpr 2019
    """
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=x2ms_adapter.nn.BatchNorm2d, use_dropout=False, n_blocks=9, padding_type='reflect'):
        assert(n_blocks >= 0)
        ...
        ngf_in = input_nc
        ngf_out = ngf * 2 ** 0
        self.encoder_layer1 = x2ms_adapter.nn.Sequential(
            # state size: 64 channel
            x2ms_adapter.nn.Conv2d(ngf_in, ngf_out, kernel_size=3,
                               stride=1, padding=1, bias=use_bias),
            x2ms_adapter.nn.AvgPool2d(kernel_size=3, stride=2, padding=1),
            norm_layer(ngf_out),
        ...
        ngf_in = ngf_out
        ngf_out = ngf_out * 2
        self.encoder_layer2 = x2ms_adapter.nn.Sequential(
        ...
        ngf_in = ngf_out
        ngf_out = ngf_out * 2
        self.encoder_layer3 = x2ms_adapter.nn.Sequential(
        ...
        ngf_in = ngf_out
        ngf_out = ngf_out * 2
        self.encoder_layer4 = x2ms_adapter.nn.Sequential(
        ...
        self.encoder_resblock_layer = x2ms_adapter.nn.Sequential(*model)
        ...
        for i in range(5):  # add ResNet blocks
            model += [ResnetBlock(ngf_in, padding_type=padding_type,
                                  norm_layer=norm_layer, use_dropout=True, use_bias=use_bias)]
        self.decoder_resblock_layer = x2ms_adapter.nn.Sequential(*model)
    ...
    def construct(self, input):
        ...
        output = x2ms_adapter.cat((output, self.e4_out),1)
        output = self.decoder_layer1(output)
        output = x2ms_adapter.cat((output, self.e3_out), 1)
        output = self.decoder_layer2(output)
        output = x2ms_adapter.cat((output, self.e2_out), 1)
        output = self.decoder_layer3(output)
        output = x2ms_adapter.cat((output, self.e1_out), 1)
...
class UnetAFL_v3(nn.Cell):
    ...
    def construct(self, gene_input, disc_out=None, alpha=None):
        if disc_out is None:
            ...
            output = x2ms_adapter.cat((output, e4_out),1)
            output = self.decoder_layer1(output)
            output = x2ms_adapter.cat((output, e3_out), 1)
            output = self.decoder_layer2(output)
            output = x2ms_adapter.cat((output, e2_out), 1)
            output = self.decoder_layer3(output)
            output = x2ms_adapter.cat((output, e1_out), 1)
            output = self.decoder_layer4(output)
            gen_img = output[:, 0:3, ::]
            gen_seg = output[:, 3:6, ::]
            ...
            feedback_input_new = x2ms_adapter.cat((disc_out[0], feedback_input), 1)
            ...
            feedback_input = x2ms_adapter.cat((feedback_input, gene_input[1]), 1)
            feedback_input_new = x2ms_adapter.cat((disc_out[1], feedback_input), 1)
            ...
            feedback_input = x2ms_adapter.cat((feedback_input, gene_input[2]), 1)
            feedback_input_new = x2ms_adapter.cat((disc_out[2], feedback_input), 1)
            ...
            feedback_input = x2ms_adapter.cat((feedback_input, gene_input[3]), 1)
            feedback_input_new = x2ms_adapter.cat((disc_out[3], feedback_input), 1)
            ...
            feedback_input = x2ms_adapter.cat((feedback_input, gene_input[4]), 1)
            feedback_input_new = x2ms_adapter.cat((disc_out[4], feedback_input), 1)
...
class panoGAN_baseline_G(nn.Cell):
    """
    network structure of Feedback Adversarial Learning - cvpr 2019
    """
    def __init__(self, input_nc, output_nc, ngf=64, ndf=64, norm_layer=x2ms_adapter.nn.BatchNorm2d, use_dropout=False,
    ...
    def construct(self, gene_input):
        ...
        gen_img = output[:, 0:3, ::]
        gen_seg = []
        if x2ms_adapter.tensor_api.size(output)[1] == 6:
            gen_seg = output[:, 3:6, ::]
...
class ResnetAFLGenerator(nn.Cell):
    """Resnet-based generator that consists of Adversarial Feedback Loop.
    """

    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=x2ms_adapter.nn.BatchNorm2d, use_dropout=False,
    ...
    def _forward_afl(self, inner_fea):
        bn1 = x2ms_adapter.nn.BatchNorm2d(self.ngf * 8).to(0)
        bn2 = x2ms_adapter.nn.BatchNorm2d(self.ngf * 4).to(0)
        bn3 = x2ms_adapter.nn.BatchNorm2d(self.ngf * 2).to(0)
        bn4 = x2ms_adapter.nn.BatchNorm2d(self.ngf * 1).to(0)
        out = inner_fea  # resBlock output
        temp_out = out + self.alpha1 * self.netGA.trans_block0(x2ms_adapter.cat([self.netGA.feedback0, out], 1))
        out = self.main.decoder_layer1(bn1(temp_out))
        temp_out = out + self.alpha2 * self.netGA.trans_block1(x2ms_adapter.cat([self.netGA.feedback1, out], 1))
        out = self.main.decoder_layer2(bn2(temp_out))
        temp_out = out + self.alpha3 * self.netGA.trans_block2(x2ms_adapter.cat([self.netGA.feedback2, out], 1))
        out = self.main.decoder_layer3(bn3(temp_out))
        temp_out = out + self.alpha4 * self.netGA.trans_block3(x2ms_adapter.cat([self.netGA.feedback3, out], 1))
    ...
    def _forward_afl_v2(self, inner_fea):
        ...
        out = self.main.decoder_layer1(
            x2ms_adapter.cat([out, self.alpha1 * self.netGA.trans_block0(x2ms_adapter.cat([self.netGA.feedback0, out], 1))], 1))
        out = self.main.decoder_layer2(
            x2ms_adapter.cat([out, self.alpha2 * self.netGA.trans_block1(x2ms_adapter.cat([self.netGA.feedback1, out], 1))], 1))
        out = self.main.decoder_layer3(
            x2ms_adapter.cat([out, self.alpha3 * self.netGA.trans_block2(x2ms_adapter.cat([self.netGA.feedback2, out], 1))], 1))
        out = self.main.decoder_layer4(
            x2ms_adapter.cat([out, self.alpha4 * self.netGA.trans_block3(x2ms_adapter.cat([self.netGA.feedback3, out], 1))], 1))
        return out

    def construct(self, x):
        gen_output = self.main(x)
        fake_AB = x2ms_adapter.cat((x, gen_output),
                            1)  # we use conditional GANs; we need to feed both input and output to the discriminator
        d_out_bk = self.netD(fake_AB)  # Fake; stop backprop to the generator by detaching fake_B
...
class ResnetAFLGenerator_FAL(nn.Cell):
    """Resnet-based generator that consists of Adversarial Feedback Loop. with Feedback Adversarial Learning - cvpr19
    """
    ...
    def _forward_afl(self, inner_fea):
        out = inner_fea  # resBlock output
        temp_out = out + self.alpha[0] * self.netGA.trans_block1(x2ms_adapter.cat([self.netGA.feedback1, out], 1))
        out = self.main.decoder_resblock_layer(self.norm_layer1(temp_out))
        temp_out = out + self.alpha[1] * self.netGA.trans_block2(x2ms_adapter.cat([self.netGA.feedback2, out], 1))
        out = self.main.decoder_layer1(self.norm_layer2(temp_out))
        temp_out = out + self.alpha[2] * self.netGA.trans_block3(x2ms_adapter.cat([self.netGA.feedback3, out], 1))
        out = self.main.decoder_layer2(self.norm_layer3(temp_out))
        temp_out = out + self.alpha[3] * self.netGA.trans_block4(x2ms_adapter.cat([self.netGA.feedback4, out], 1))
        out = self.main.decoder_layer3(self.norm_layer4(temp_out))
        temp_out = out + self.alpha[4] * self.netGA.trans_block5(x2ms_adapter.cat([self.netGA.feedback5, out], 1))
...
class MainNet(nn.Cell):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=x2ms_adapter.nn.BatchNorm2d, use_dropout=False, n_blocks=6,
        ...
        self.encoder_layer1 = x2ms_adapter.nn.Sequential(
            # state size: 64 channel
            x2ms_adapter.nn.Conv2d(input_nc, ngf, kernel_size=4,
        ...
        self.encoder_layer2 = x2ms_adapter.nn.Sequential(
            ...
            x2ms_adapter.nn.Conv2d(ngf, ngf * 2, kernel_size=4,
                      stride=2, padding=1, bias=use_bias),
            norm_layer(ngf),
            x2ms_adapter.nn.ReLU(inplace=True)
        )
        self.encoder_layer3 = x2ms_adapter.nn.Sequential(
            # state size: 256 channel
            x2ms_adapter.nn.Conv2d(ngf * 2, ngf * 4, kernel_size=4,
                      stride=2, padding=1, bias=use_bias),
            norm_layer(ngf),
            x2ms_adapter.nn.ReLU(inplace=True)
        )
        self.encoder_layer4 = x2ms_adapter.nn.Sequential(
            ...
            x2ms_adapter.nn.Conv2d(ngf * 4, ngf * 8, kernel_size=4,
                      stride=2, padding=1, bias=use_bias),
            norm_layer(ngf),
            x2ms_adapter.nn.ReLU(inplace=True)
        )
        ...
        for i in range(n_blocks):  # add ResNet blocks
            model += [ResnetBlock(ngf * 8, padding_type=padding_type,
                                  norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias)]
        self.resnetBlock = x2ms_adapter.nn.Sequential(*model)
        """
...
class TransBlockDual(nn.Cell):
    def __init__(self, afl_type=1, input_nc=6, output_nc=3):
        super(TransBlockDual, self).__init__()
        if afl_type == 1:
            self.main = x2ms_adapter.nn.Sequential(
                x2ms_adapter.nn.Conv2d(input_nc, output_nc, kernel_size=3, stride=1, padding=1),
                nn.InstanceNorm2d(output_nc),
                x2ms_adapter.nn.ReLU(True),
                x2ms_adapter.nn.Conv2d(output_nc, output_nc, kernel_size=3, stride=1, padding=1),
                nn.InstanceNorm2d(output_nc),
                x2ms_adapter.nn.ReLU(True))
        elif afl_type == 2:
            pass
...
class Parm:
    def __init__(self, ngf=64, norm_layer=x2ms_adapter.nn.BatchNorm2d, use_dropout=False, afl_type=1, padding_type='reflect'):
        self.size = ngf
...
class ResnetBlock(nn.Cell):
    ...
    def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias):
        ...
        conv_block += [x2ms_adapter.nn.Conv2d(dim, dim, kernel_size=3, padding=p, bias=use_bias), norm_layer(dim), x2ms_adapter.nn.ReLU(True)]
        if use_dropout:
            conv_block += [x2ms_adapter.nn.Dropout(0.5)]
        ...
        if padding_type == 'reflect':
            ...
            raise NotImplementedError('padding [%s] is not implemented' % padding_type)
        conv_block += [x2ms_adapter.nn.Conv2d(dim, dim, kernel_size=3, padding=p, bias=use_bias), norm_layer(dim)]

        return x2ms_adapter.nn.Sequential(*conv_block)
...
class UnetGenerator(nn.Cell):
    ...
    def __init__(self, input_nc, output_nc, num_downs, ngf=64, norm_layer=x2ms_adapter.nn.BatchNorm2d, use_dropout=False):
...
class UnetSkipConnectionBlock(nn.Cell):
    ...
    def __init__(self, outer_nc, inner_nc, input_nc=None,
                 submodule=None, outermost=False, innermost=False, norm_layer=x2ms_adapter.nn.BatchNorm2d, use_dropout=False):
        ...
        if input_nc is None:
            input_nc = outer_nc
        downconv = x2ms_adapter.nn.Conv2d(input_nc, inner_nc, kernel_size=4,
                             stride=2, padding=1, bias=use_bias)
        downrelu = nn.LeakyReLU(0.2, True)
        downnorm = norm_layer(inner_nc)
        uprelu = x2ms_adapter.nn.ReLU(True)
        ...
        if outermost:
            ...
            if use_dropout:
                model = down + [submodule] + up + [x2ms_adapter.nn.Dropout(0.5)]
            else:
                model = down + [submodule] + up
    ...
    def construct(self, x):
        if self.outermost:
            ...
            return x2ms_adapter.cat([x, self.model(x)], 1)


class Unet6cGenerator(nn.Cell):
...
class UnetAFL_4a(nn.Cell):
    ...
    def _forward_afl(self, fea_list):
        ...
        input_fea = input_fea + self.alpha[0] * self.norm_layer0(
            self.netGA.trans_block0(x2ms_adapter.cat([self.netGA.feedback0, input_fea], 1)))
        ...
        input_fea = input_fea + self.alpha[1] * self.norm_layer1(
            self.netGA.trans_block1(x2ms_adapter.cat([self.netGA.feedback1, input_fea], 1)))
        ...
        input_fea = input_fea + self.alpha[2] * self.norm_layer2(
            self.netGA.trans_block2(x2ms_adapter.cat([self.netGA.feedback2, input_fea], 1)))
        ...
        input_fea = input_fea + self.alpha[3] * self.norm_layer3(
            self.netGA.trans_block3(x2ms_adapter.cat([self.netGA.feedback3, input_fea], 1)))
        ...
        input_fea = input_fea + self.alpha[4] * self.norm_layer4(
            self.netGA.trans_block4(x2ms_adapter.cat([self.netGA.feedback4, input_fea], 1)))
...
class UNetDown(nn.Cell):
    def __init__(self, in_size, out_size, normalize=True, dropout=0.0):
        ...
        layers = [x2ms_adapter.nn.Conv2d(in_size, out_size, 4, 2, 1, bias=True)]
        if normalize:
            layers.append(nn.InstanceNorm2d(out_size))
        layers.append(nn.LeakyReLU(0.2))
        if dropout:
            layers.append(x2ms_adapter.nn.Dropout(dropout))
        self.model = x2ms_adapter.nn.Sequential(*layers)
...
class UNetUp(nn.Cell):
    ...
    def construct(self, x, skip_input):
        x = self.model(x)
        x = x2ms_adapter.cat((x, skip_input), 1)

        return x


class BasicUNetGenerator(nn.Cell):
    ...
    def __init__(self, input_nc=3, output_nc=3):
        ...
        self.final = x2ms_adapter.nn.Sequential(
            x2ms_adapter.nn.Upsample(scale_factor=2),
            x2ms_adapter.nn.ZeroPad2d((1, 0, 1, 0)),
            x2ms_adapter.nn.Conv2d(128, output_nc, 4, padding=1),
            nn.Tanh(),
        )
...
class NLayerDiscriminator(nn.Cell):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=x2ms_adapter.nn.BatchNorm2d):
        ...
        kw = 4
        padw = 1
        sequence = [x2ms_adapter.nn.Conv2d(input_nc, ndf, kernel_size=kw, stride=2, padding=padw), nn.LeakyReLU(0.2, True)]
        nf_mult = 1
        nf_mult_prev = 1
        for n in range(1, n_layers):  # gradually increase the number of filters
            ...
            nf_mult = min(2 ** n, 8)
            sequence += [
                x2ms_adapter.nn.Conv2d(ndf * nf_mult_prev, ndf * nf_mult, kernel_size=kw, stride=2, padding=padw, bias=use_bias),
        ...
        sequence += [
            x2ms_adapter.nn.Conv2d(ndf * nf_mult_prev, ndf * nf_mult, kernel_size=kw, stride=1, padding=padw, bias=use_bias),
            norm_layer(ndf * nf_mult),
            nn.LeakyReLU(0.2, True)
        ]

        sequence += [
            x2ms_adapter.nn.Conv2d(ndf * nf_mult, 1, kernel_size=kw, stride=1, padding=padw)]  # output 1 channel prediction map
        self.model = x2ms_adapter.nn.Sequential(*sequence)
...
class PixelDiscriminator(nn.Cell):
    ...
    def __init__(self, input_nc, ndf=64, norm_layer=x2ms_adapter.nn.BatchNorm2d):
        ...
        self.net = [
            x2ms_adapter.nn.Conv2d(input_nc, ndf, kernel_size=1, stride=1, padding=0),
            nn.LeakyReLU(0.2, True),
            x2ms_adapter.nn.Conv2d(ndf, ndf * 2, kernel_size=1, stride=1, padding=0, bias=use_bias),
            norm_layer(ndf * 2),
            nn.LeakyReLU(0.2, True),
            x2ms_adapter.nn.Conv2d(ndf * 2, 1, kernel_size=1, stride=1, padding=0, bias=use_bias)]

        self.net = x2ms_adapter.nn.Sequential(*self.net)
...
class Discriminator_UNet(nn.Cell):
    """Defines a PatchGAN discriminator"""
    ...
    def __init__(self, input_nc, ndf=64, norm_layer=x2ms_adapter.nn.BatchNorm2d):
        ...
        self.l1 = x2ms_adapter.nn.Sequential(  # input size is 256
            x2ms_adapter.nn.Conv2d(input_nc, ndf, kernel_size=4, stride=2, padding=1, bias=use_bias),
            nn.LeakyReLU(0.2, inplace=True))
        self.l2 = x2ms_adapter.nn.Sequential(  # input size is 128
            x2ms_adapter.nn.Conv2d(ndf, ndf * 2, kernel_size=4, stride=2, padding=1, bias=use_bias),
            nn.LeakyReLU(0.2, inplace=True))
        self.l3 = x2ms_adapter.nn.Sequential(  # input size is 64
            x2ms_adapter.nn.Conv2d(ndf * 2, ndf * 4, kernel_size=4, stride=2, padding=1, bias=use_bias),
            nn.LeakyReLU(0.2, inplace=True))
        self.l4 = x2ms_adapter.nn.Sequential(  # input size is 32
            x2ms_adapter.nn.Conv2d(ndf * 4, ndf * 8, kernel_size=4, stride=2, padding=1, bias=use_bias),
            nn.LeakyReLU(0.2, inplace=True))
        self.l5 = x2ms_adapter.nn.Sequential(  # input size is 16
            x2ms_adapter.nn.Conv2d(ndf * 8, ndf * 8, kernel_size=4, stride=2, padding=1, bias=use_bias),
            nn.LeakyReLU(0.2, inplace=True))
        self.l6 = x2ms_adapter.nn.Sequential(  # input size is 15
            x2ms_adapter.nn.Conv2d(ndf * 8, 1, kernel_size=4, stride=1, padding=1)
        )
...
class AFLDiscriminator_FAL(nn.Cell):
    ...
    def __init__(self, input_nc, ndf=64, norm_layer=x2ms_adapter.nn.BatchNorm2d):
        ...
        ndf_out = ndf
        self.l1 = x2ms_adapter.nn.Sequential(  # input size is 256
            x2ms_adapter.nn.Conv2d(ndf_in, ndf_out, kernel_size=3, stride=1, padding=1, bias=use_bias),
            x2ms_adapter.nn.AvgPool2d(kernel_size=3, stride=2, padding=1),
            norm_layer(ndf_out),
            nn.LeakyReLU(0.2, inplace=True))
        ...
        ndf_in = ndf_out
        ndf_out = ndf_in * 2
        self.l2 = x2ms_adapter.nn.Sequential(  # input size is 256
            x2ms_adapter.nn.Conv2d(ndf_in, ndf_out, kernel_size=3, stride=1, padding=1, bias=use_bias),
            x2ms_adapter.nn.AvgPool2d(kernel_size=3, stride=2, padding=1),
            norm_layer(ndf_out),
            nn.LeakyReLU(0.2, inplace=True))
        ...
        ndf_in = ndf_out
        ndf_out = ndf_in * 2
        self.l3 = x2ms_adapter.nn.Sequential(  # input size is 256
            x2ms_adapter.nn.Conv2d(ndf_in, ndf_out, kernel_size=3, stride=1, padding=1, bias=use_bias),
            x2ms_adapter.nn.AvgPool2d(kernel_size=3, stride=2, padding=1),
            norm_layer(ndf_out),
            nn.LeakyReLU(0.2, inplace=True))
        ...
        ndf_in = ndf_out
        ndf_out = ndf_in * 2
        self.l4 = x2ms_adapter.nn.Sequential(  # input size is 256
            x2ms_adapter.nn.Conv2d(ndf_in, ndf_out, kernel_size=3, stride=1, padding=1, bias=use_bias),
            x2ms_adapter.nn.AvgPool2d(kernel_size=3, stride=2, padding=1),
            norm_layer(ndf_out),
            nn.LeakyReLU(0.2, inplace=True))
        ...
        ndf_in = ndf_out
        ndf_out = ndf_in
        self.l5 = x2ms_adapter.nn.Sequential(  # input size is 256
            x2ms_adapter.nn.Conv2d(ndf_in, ndf_out, kernel_size=3, stride=1, padding=1, bias=use_bias),
        ...
        self.l6 = x2ms_adapter.nn.Sequential(  # input size is 15
            # nn.ReLU(True),
            x2ms_adapter.nn.Conv2d(ndf_in, ndf_out, kernel_size=3, stride=1, padding=0, bias=use_bias)
        )
...
class Discriminator_PatchGAN_Feedback(nn.Cell):
    """the discriminator is borrowed from Feedback Adversarial Learning -cvpr19"""
    def __init__(self, input_nc, ndf=64, norm_layer=x2ms_adapter.nn.BatchNorm2d):
    ...
    def construct(self, input):
        ...
        l6out = self.l6(out)
        return l6out, [l5out, l4out, l3out, l2out, l1out]

class FeaturePyramidDiscriminator(nn.Cell):
    """the discriminator is borrowed from Feedback Adversarial Learning -cvpr19"""
    def __init__(self, input_nc, ndf=64, norm_layer=x2ms_adapter.nn.BatchNorm2d, disc_nc_scale=[1,1,1,1,1]):
        ...
        #-------- top-down pathway:
        ndf_out = ndf * 2
        self.lat1 = x2ms_adapter.nn.Sequential(
            x2ms_adapter.nn.Conv2d(ndf, ndf_out, kernel_size=1),
            norm_layer(ndf_out),
            nn.LeakyReLU(0.2, True))
        self.lat2 = x2ms_adapter.nn.Sequential(
            x2ms_adapter.nn.Conv2d(ndf * 2, ndf_out, kernel_size=1),
            norm_layer(ndf_out),
            nn.LeakyReLU(0.2, True))
        self.lat3 = x2ms_adapter.nn.Sequential(
            x2ms_adapter.nn.Conv2d(ndf * 4, ndf_out, kernel_size=1),
            norm_layer(ndf_out),
            nn.LeakyReLU(0.2, True))
        self.lat4 = x2ms_adapter.nn.Sequential(
            x2ms_adapter.nn.Conv2d(ndf * 8, ndf_out, kernel_size=1),
            norm_layer(ndf_out),
            nn.LeakyReLU(0.2, True))
        self.lat5 = x2ms_adapter.nn.Sequential(
            x2ms_adapter.nn.Conv2d(ndf * 8, ndf_out, kernel_size=1),
            norm_layer(ndf_out),
            nn.LeakyReLU(0.2, True))

        # upsampling
        self.up = x2ms_adapter.nn.Upsample(scale_factor=2, mode='bilinear')

        # final layers
        ndf_in = ndf_out
        ndf_out = ndf * disc_nc_scale[0]
        self.final1 = x2ms_adapter.nn.Sequential(
            x2ms_adapter.nn.Conv2d(ndf_in, ndf_out, kernel_size=1),
            norm_layer(ndf_out),
            nn.LeakyReLU(0.2, True))
        ndf_out = ndf * disc_nc_scale[1]
        self.final2 = x2ms_adapter.nn.Sequential(
            x2ms_adapter.nn.Conv2d(ndf_in, ndf_out, kernel_size=1),
            norm_layer(ndf_out),
            nn.LeakyReLU(0.2, True))
        ndf_out = ndf * disc_nc_scale[2]
        self.final3 = x2ms_adapter.nn.Sequential(
            x2ms_adapter.nn.Conv2d(ndf_in, ndf_out, kernel_size=1),
            norm_layer(ndf_out),
            nn.LeakyReLU(0.2, True))
        ndf_out = ndf * disc_nc_scale[3]
        self.final4 = x2ms_adapter.nn.Sequential(
            x2ms_adapter.nn.Conv2d(ndf_in, ndf_out, kernel_size=1),
            norm_layer(ndf_out),
            nn.LeakyReLU(0.2, True))
        ndf_out = ndf * disc_nc_scale[4]
        self.final5 = x2ms_adapter.nn.Sequential(
            x2ms_adapter.nn.Conv2d(ndf_in, ndf_out, kernel_size=1),
            norm_layer(ndf_out),
            nn.LeakyReLU(0.2, True))
        ...
        ndf_in = ndf_out
        ndf_out = ndf_in
        self.tf = x2ms_adapter.nn.Conv2d(ndf_in, 1, kernel_size=1)
        self.seg = x2ms_adapter.nn.Conv2d(ndf_in, ndf_out, kernel_size=1)
    ...
    def construct(self, input):
        ...
        feat = [feat35, feat34, feat33, feat32, feat31]
