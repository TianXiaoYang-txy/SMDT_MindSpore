...
class panoganModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            self.optimizer_D_seg = optim_register.adam(x2ms_adapter.get_params(self.netD_seg), lr=opt.lr, betas=(opt.beta1, 0.999))
    ...
    def forward(self):
        ...
        for i in range(0, self.loop_count):
            ...
            #---------img loss ------------------------
            # fake
            fake_AB = x2ms_adapter.cat((self.img_A, fake_B), 1)  # we use conditional GANs; we need to feed both input and output to the discriminator
            _, _, disc_out_img = self.netD_img(fake_AB)  # Fake; stop backprop to the generator by detaching fake_B

            # ---------seg loss ------------------------
            # fake
            fake_AD = x2ms_adapter.cat((self.img_A, fake_D), 1)  # we use conditional GANs; we need to feed both input and output to the discriminator
            ...
            for j in range(len(disc_out_img)):
                disc_out_img[j] = x2ms_adapter.cat((disc_out_img[j], disc_out_seg[j]), 1)
    ...
    def backward_D(self):
        ...
        # ---------D loss ------------------------
            # fake image
            fake_AB = x2ms_adapter.cat((self.img_A, self.fake_B[j]),
    ...
    def optimize_parameters(self):
        x2ms_adapter.forward(self)  # compute fake images: G(A)
        # update D
    ...
    def confuse_pred_embed(self, pred_embed_fake, pred_embed_real=None, used_for_disc=True):
        ...
        for i in range(len(pred_fake_img)):
            mul = ops.Mul()
            pred_fake_img[i] += x2ms_adapter.tensor_api.sum(mul(embed_fake_img[i], embed_real_seg[i]), dim=1, keepdim=True)
            pred_fake_seg[i] += x2ms_adapter.tensor_api.sum(mul(embed_real_img[i], embed_fake_seg[i]), dim=1, keepdim=True)

        if used_for_disc:
            for i in range(len(pred_fake_img)):
                mul = ops.Mul()
                pred_real_i = x2ms_adapter.tensor_api.sum(mul(embed_real_img[i], embed_real_seg[i]), dim=1, keepdim=True)
