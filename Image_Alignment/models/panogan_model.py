...
class panoganModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            self.criterionL1 = mindspore.nn.L1Loss()
            # initialize optimizers; schedulers will be automatically created by function <BaseModel.setup>.
            self.optimizer_G = mindspore.nn.Adam(self.netG.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999))
            self.optimizer_D_img = mindspore.nn.Adam(self.netD_img.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999))
            self.optimizer_D_seg = mindspore.nn.Adam(self.netD_seg.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999))
            # self.optimizer_F_img = mindspore.nn.Adam(self.netF_img.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999))
            # self.optimizer_F_seg = mindspore.nn.Adam(self.netF_seg.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999))
    ...
    def forward(self):
        ...
        for i in range(0, self.loop_count):
            ...
            #---------img loss ------------------------
            # fake
            fake_AB = mindspore.ops.Concat((self.img_A, fake_B), 1)  # we use conditional GANs; we need to feed both input and output to the discriminator
            _, _, disc_out_img = self.netD_img(fake_AB)  # Fake; stop backprop to the generator by detaching fake_B

            # ---------seg loss ------------------------
            # fake
            fake_AD = mindspore.ops.Concat((self.img_A, fake_D), 1)  # we use conditional GANs; we need to feed both input and output to the discriminator
            ...
            for j in range(len(disc_out_img)):
                disc_out_img[j] = mindspore.ops.Concat((disc_out_img[j], disc_out_seg[j]), 1)
    ...
    def backward_D(self):
        ...
        # ---------D loss ------------------------
            # fake image
            fake_AB = mindspore.ops.Concat((self.img_A, self.fake_B[j]),
    ...
    def confuse_pred_embed(self, pred_embed_fake, pred_embed_real=None, used_for_disc=True):
        ...
        for i in range(len(pred_fake_img)):
            pred_fake_img[i] += mindspore.ops.Mul(embed_fake_img[i], embed_real_seg[i]).sum(dim=1, keepdim=True)
            pred_fake_seg[i] += mindspore.ops.Mul(embed_real_img[i], embed_fake_seg[i]).sum(dim=1, keepdim=True)

        if used_for_disc:
            for i in range(len(pred_fake_img)):
                pred_real_i = mindspore.ops.Mul(embed_real_img[i], embed_real_seg[i]).sum(dim=1, keepdim=True)
