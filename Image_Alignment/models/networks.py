...
def get_norm_layer(norm_type='instance'):
    ...
    """
    if norm_type == 'batch':
        # norm_layer = functools.partial(mindspore.nn.BatchNorm2d, affine=True, track_running_stats=True)
        norm_layer = functools.partial(mindspore.nn.BatchNorm2d, affine=True)
    elif norm_type == 'instance':
        # norm_layer = functools.partial(mindspore.nn.InstanceNorm2d, affine=False, track_running_stats=False)
        norm_layer = functools.partial(mindspore.nn.InstanceNorm2d, affine=False)
    elif norm_type == 'none':
        norm_layer = None
    else:
...
def get_scheduler(optimizer, opt):
    ...
    if opt.lr_policy == 'linear':
        ...
        scheduler = mindspore.nn.piecewise_constant_lr(optimizer, step_size=opt.lr_decay_iters, gamma=0.1)
    elif opt.lr_policy == 'plateau':
        scheduler = lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.2, threshold=0.01, patience=5)
    elif opt.lr_policy == 'cosine':
        scheduler = mindspore.nn.learning_rate_schedule.CosineDecayLR(optimizer, T_max=opt.niter, eta_min=0)
    else:
...
def init_weights(net, init_type='normal', init_gain=0.02):
    ...
    def init_func(m):  # define the initialization function
        ...
        if hasattr(m, 'weight') and (classname.find('Conv') != -1 or classname.find('Linear') != -1):
            if init_type == 'normal':
                mindspore.common.initializer.Normal(m.weight.data, 0.0, init_gain)
            elif init_type == 'xavier':
                # init.xavier_normal_(m.weight.data, gain=init_gain)
                mindspore.common.initializer.XavierUniform(m.weight.data, gain=init_gain)
            elif init_type == 'kaiming':
                mindspore.common.initializer.HeNormal(m.weight.data, a=0, mode='fan_in')
            elif init_type == 'orthogonal':
                mindspore.common.initializer.Orthogonal(m.weight.data, gain=init_gain)
            else:
                raise NotImplementedError('initialization method [%s] is not implemented' % init_type)
            if hasattr(m, 'bias') and m.bias is not None:
                mindspore.common.initializer.Constant(m.bias.data, 0.0)
        elif classname.find(
                'BatchNorm2d') != -1:  # BatchNorm Layer's weight is not a matrix; only normal distribution applies.
            mindspore.common.initializer.Normal(m.weight.data, 1.0, init_gain)
            mindspore.common.initializer.Constant(m.bias.data, 0.0)
...
# Classes
##############################################################################
class GANLoss(mindspore.nn.Cell):
    ...
    def __init__(self, gan_mode, target_real_label=1.0, target_fake_label=0.0):
        ...
        self.register_buffer('real_label', mindspore.Tensor(target_real_label))
        self.register_buffer('fake_label', mindspore.Tensor(target_fake_label))
        self.gan_mode = gan_mode
        if gan_mode == 'lsgan':
            self.loss = mindspore.nn.MSELoss()
...
def cal_gradient_penalty(netD, real_data, fake_data, device, type='mixed', constant=1.0, lambda_gp=10.0):
    ...
    if lambda_gp > 0.0:
        if type == 'real':  # either use real images, fake images, or a linear interpolation of two.
            ...
            alpha = mindspore.ops.UniformReal(real_data.shape[0], 1, device=device)
        ...
        #                                 grad_outputs=mindspore.ops.Ones(disc_interpolates.size()).to(device),
        #                                 create_graph=True, retain_graph=True, only_inputs=True)
        gradients = mindspore.ops.GradOperation(get_all=True, get_by_list=True, sens_param=True)
...
class XForkGenerator(mindspore.nn.Cell):
    ...
    def __init__(self, input_nc, norm_layer, use_dropout):
        ...
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func == mindspore.nn.InstanceNorm2d
        else:
            use_bias = norm_layer == mindspore.nn.InstanceNorm2d
...
class XFork_Shared_Encoder_Block(mindspore.nn.Cell):
    def __init__(self, input_nc, norm_layer, use_dropout, use_bias):
        ...
        nc_output_tmp = 64
        conv = mindspore.nn.Conv2d(nc_input_tmp, nc_output_tmp, kernel_size=4, stride=2, padding=1, has_bias=use_bias, pad_mode='pad')  # 128x128
        act = mindspore.nn.LeakyReLU(negative_slope)
        generator_block += [conv, act]

        nc_input_tmp = nc_output_tmp
        nc_output_tmp = 128
        conv = mindspore.nn.Conv2d(nc_input_tmp, nc_output_tmp, kernel_size=4, stride=2, padding=1, has_bias=use_bias, pad_mode='pad')  # 64x64
        ...
        nc_input_tmp = nc_output_tmp
        nc_output_tmp = 256
        conv = mindspore.nn.Conv2d(nc_input_tmp, nc_output_tmp, kernel_size=4, stride=2, padding=1, has_bias=use_bias, pad_mode='pad')  # 32x32
        norm = norm_layer(nc_output_tmp)  # swap norm and act for fine-turning
        act = mindspore.nn.LeakyReLU(negative_slope)
        generator_block += [conv, norm, act]
        ...
        for i in range(1, 5):  # 16x16 -> 8x8 -> 4x4 -> 2x2 ->1x1
            nc_input_tmp = nc_output_tmp
            nc_output_tmp = 512
            conv = mindspore.nn.Conv2d(nc_input_tmp, nc_output_tmp, kernel_size=4, stride=2, padding=1, has_bias=use_bias, pad_mode='pad')
            norm = norm_layer(nc_output_tmp)  # swap norm and act for fine-turning
            act = mindspore.nn.LeakyReLU(negative_slope)
            generator_block += [conv, norm, act]
        ...
        nc_output_tmp = 512
        conv = mindspore.nn.Conv2d(nc_input_tmp, nc_output_tmp, kernel_size=4, stride=2, padding=1, has_bias=use_bias, pad_mode='pad')
        norm = mindspore.nn.BatchNorm2d(nc_output_tmp)
        act = mindspore.nn.ReLU(True)
        generator_block += [conv, norm, act]

        self.model = mindspore.nn.SequentialCellCell(*generator_block)
...
class XFork_Shared_Decoder_Block(mindspore.nn.Cell):
    def __init__(self, input_nc, norm_layer, use_dropout, use_bias):
        ...
        for i in range(3):
            if i == 0:
                ...
                nc_input_tmp = nc_output_tmp
            nc_output_tmp = 512
            tconv = mindspore.nn.Conv2dTranspose(nc_input_tmp, nc_output_tmp, kernel_size=4, stride=2, padding=1,
                                       has_bias=use_bias, pad_mode='pad')  #
            norm = norm_layer(nc_output_tmp)  # swap norm and act for fine-turning
            drop = mindspore.mindspore.nn.Dropout(0.5)
            act = mindspore.nn.ReLU(True)
        ...
        for i in range(3):
            ...
            tconv = mindspore.nn.Conv2dTranspose(nc_input_tmp, nc_output_tmp, kernel_size=4, stride=2, padding=1,
                                       has_bias=use_bias, pad_mode='pad')  # upsampling by 2 times: 64 x 256
            norm = norm_layer(nc_output_tmp)  # swap norm and act for fine-turning
            drop = mindspore.mindspore.nn.Dropout(0.5)
            act = mindspore.nn.ReLU(True)
...
class XFork_Separate_Decoder_Block1(mindspore.nn.Cell):
...
class XFork_Separate_Decoder_Block2(mindspore.nn.Cell):
    def __init__(self, input_nc, output_nc, norm_layer, use_dropout, use_bias):
        ...
        self.model = mindspore.nn.SequentialCell(*generator_block)

    def forward(self, x):
        return self.model(x)


class ResnetGenerator(mindspore.nn.Cell):
    """Resnet-based generator that consists of Resnet blocks between a few downsampling/upsampling operations.

    We adapt Torch code and idea from Justin Johnson's neural style transfer project(https://github.com/jcjohnson/fast-neural-style)
    """

    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=mindspore.nn.BatchNorm2d, use_dropout=False, n_blocks=6,
        ...
        model = [mindspore.nn.Pad(3),
                 mindspore.nn.Conv2d(input_nc, ngf, kernel_size=7, padding=0, has_bias=use_bias, pad_mode='pad'),
                 norm_layer(ngf),
                 mindspore.nn.ReLU(True)]

        n_downsampling = 2
        for i in range(n_downsampling):  # add downsampling layers
            mult = 2 ** i
            model += [mindspore.nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=2, padding=1, has_bias=use_bias, pad_mode='pad'),
        ...
        for i in range(n_downsampling):  # add upsampling layers
            ...
            model += [mindspore.nn.Conv2dTranspose(ngf * mult, int(ngf * mult / 2),
                      ...
                      mindspore.nn.ReLU(True)]
        model += [mindspore.nn.Pad(3)]
        model += [mindspore.nn.Conv2d(ngf, output_nc, kernel_size=7, padding=0)]
        model += [mindspore.nn.Tanh()]

        self.model = mindspore.nn.SequentialCell(*model)
...
#--------------------- UnetAFL_4b----------------------------------------
class UnetAFL_4b(mindspore.nn.Cell):
    """Resnet-based generator that consists of Adversarial Feedback Loop. with Feedback Adversarial Learning - cvpr19
    """
    def __init__(self, input_nc, output_nc, ngf=64, ndf=64, norm_layer=mindspore.nn.BatchNorm2d, use_dropout=False,
    ...
    def _forward_afl(self, fea_list):
        # input_fea = mindspore.ops.Concat((decoder_inner_fea, fea_list[0]), 1)
        feedback_input = fea_list[0]
        feedback_input_new = mindspore.ops.Concat((self.netGA.feedback0, feedback_input), 1)
        ...
        feedback_input = mindspore.ops.Concat((feedback_input, fea_list[1]), 1)
        feedback_input_new = mindspore.ops.Concat((self.netGA.feedback1, feedback_input), 1)
        feedback_out = feedback_input + self.alpha[1] * self.norm_layer1(
            self.netGA.trans_block1(feedback_input_new))
        #feedback_out = mindspore.ops.Concat((feedback_out, fea_list[1]), 1)
        feedback_input = self.main.decoder_layer1(feedback_out)

        feedback_input = mindspore.ops.Concat((feedback_input, fea_list[2]), 1)
        feedback_input_new = mindspore.ops.Concat((self.netGA.feedback2, feedback_input), 1)
        feedback_out = feedback_input + self.alpha[2] * self.norm_layer2(
            self.netGA.trans_block2(feedback_input_new))
        #feedback_out = mindspore.ops.Concat((feedback_out, fea_list[2]), 1)
        feedback_input = self.main.decoder_layer2(feedback_out)

        feedback_input = mindspore.ops.Concat((feedback_input, fea_list[3]), 1)
        feedback_input_new = mindspore.ops.Concat((self.netGA.feedback3, feedback_input), 1)
        feedback_out = feedback_input + self.alpha[3] * self.norm_layer3(
            self.netGA.trans_block3(feedback_input_new))
        #feedback_out = mindspore.ops.Concat((feedback_out, fea_list[3]), 1)
        feedback_input = self.main.decoder_layer3(feedback_out)

        feedback_input = mindspore.ops.Concat((feedback_input, fea_list[4]), 1)
        feedback_input_new = mindspore.ops.Concat((self.netGA.feedback4, feedback_input), 1)
        feedback_out = feedback_input + self.alpha[4] * self.norm_layer4(
            self.netGA.trans_block4(feedback_input_new))
        #feedback_out = mindspore.ops.Concat((feedback_out, fea_list[4]), 1)
    ...
    def forward(self, x):
        ...
        fake_AB = mindspore.ops.Concat((x, gen_img),
        ...
        for step in range(1, self.loop_count):
            ...
            gen_img = gen_output[:, 0:3, ::]
            gen_seg = gen_output[:, 3:6, ::]
            fake_AB = mindspore.ops.Concat((x, gen_img),                                1)  # we use conditional GANs; we need to feed both input and output to the discriminator
    ...
    def showtensor(self, x):
        ...
        import mindspore
        unloader = mindspore.dataset.vision.py_transforms.ToPIL()
...
class BasicUNetGenerator_4b(mindspore.nn.Cell):
    """
    network structure of Feedback Adversarial Learning - cvpr 2019
    """
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=mindspore.nn.BatchNorm2d, use_dropout=False, n_blocks=9, padding_type='reflect'):
        assert(n_blocks >= 0)
        ...
        ngf_in = input_nc
        ngf_out = ngf * 2 ** 0
        self.encoder_layer1 = mindspore.nn.SequentialCell(
            # state size: 64 channel
            mindspore.nn.Conv2d(ngf_in, ngf_out, kernel_size=3,
                               stride=1, padding=1, has_bias=use_bias, pad_mode='pad'),
            mindspore.nn.AvgPool2d(kernel_size=3, stride=2),
            norm_layer(ngf_out),
            mindspore.nn.ReLU()
        )
        ...
        ngf_in = ngf_out
        ngf_out = ngf_out * 2
        self.encoder_layer2 = mindspore.nn.SequentialCell(
        ...
        ngf_in = ngf_out
        ngf_out = ngf_out * 2
        self.encoder_layer3 = mindspore.nn.SequentialCell(
        ...
        ngf_in = ngf_out
        ngf_out = ngf_out * 2
        self.encoder_layer4 = mindspore.nn.SequentialCell(
        ...
        self.encoder_resblock_layer = mindspore.nn.SequentialCell(*model)
        ...
        for i in range(5):  # add ResNet blocks
            model += [ResnetBlock(ngf_in, padding_type=padding_type,
                                  norm_layer=norm_layer, use_dropout=True, use_bias=use_bias)]
        self.decoder_resblock_layer = mindspore.nn.SequentialCell(*model)

        # ---------decoding layer 1 ---------------------
        ngf_out = int(ngf_in / 2)
        ngf_in = int(ngf_in * 2) # skip connection
        self.decoder_layer1 = mindspore.nn.SequentialCell(
            # state size: 256 channel
            mindspore.nn.ResizeBilinear(scale_factor=2, mode='nearest'),
            mindspore.nn.Pad(1),
            mindspore.nn.Conv2d(ngf_in, ngf_out, kernel_size=3, stride=1, padding=0, has_bias=use_bias, pad_mode='pad'),
            norm_layer(ngf_out),
            mindspore.nn.ReLU(True)
            )
        ...
        ngf_out = int(ngf_in / 2)
        ngf_in = int(ngf_in * 2)  # skip connection
        self.decoder_layer2 = mindspore.nn.SequentialCell(
            # state size: 256 channel
            mindspore.nn.ResizeBilinear(scale_factor=2, mode='nearest'),
            mindspore.nn.Pad(1),
            mindspore.nn.Conv2d(ngf_in, ngf_out, kernel_size=3, stride=1, padding=0, has_bias=use_bias, pad_mode='pad'),
        ...
        ngf_out = int(ngf_in / 2)
        ngf_in = int(ngf_in * 2)  # skip connection
        self.decoder_layer3 = mindspore.nn.SequentialCell(
            # state size: 256 channel
            mindspore.nn.ResizeBilinear(scale_factor=2, mode='nearest'),
            mindspore.nn.Pad(1),
            mindspore.nn.Conv2d(ngf_in, ngf_out, kernel_size=3, stride=1, padding=0, has_bias=use_bias, pad_mode='pad'),
        ...
        ngf_in = int(ngf_in * 2)  # skip connection
        ngf_out = output_nc
        self.decoder_layer4 = mindspore.nn.SequentialCell(
            # state size: 256 channel
            mindspore.nn.ResizeBilinear(scale_factor=2, mode='nearest'),
            mindspore.nn.Pad(1),
            mindspore.nn.Conv2d(ngf_in, ngf_out, kernel_size=3, stride=1, padding=0, has_bias=use_bias, pad_mode='pad'),
    ...
    def forward(self, input):
        ...
        output = mindspore.ops.Concat((output, self.e4_out),1)
        output = self.decoder_layer1(output)
        output = mindspore.ops.Concat((output, self.e3_out), 1)
        output = self.decoder_layer2(output)
        output = mindspore.ops.Concat((output, self.e2_out), 1)
        output = self.decoder_layer3(output)
        output = mindspore.ops.Concat((output, self.e1_out), 1)
...
class UnetAFL_v3(mindspore.nn.Cell):
    """
    network structure of Feedback Adversarial Learning - cvpr 2019
    """
    def __init__(self, input_nc, output_nc, ngf=64, ndf=64, norm_layer=mindspore.nn.BatchNorm2d, use_dropout=False,
    ...
    def forward(self, gene_input, disc_out=None, alpha=None):
        if disc_out is None:
            ...
            output = mindspore.ops.Concat((output, e4_out),1)
            output = self.decoder_layer1(output)
            output = mindspore.ops.Concat((output, e3_out), 1)
            output = self.decoder_layer2(output)
            output = mindspore.ops.Concat((output, e2_out), 1)
            output = self.decoder_layer3(output)
            output = mindspore.ops.Concat((output, e1_out), 1)
            output = self.decoder_layer4(output)
            gen_img = output[:, 0:3, ::]
            gen_seg = output[:, 3:6, ::]
            ...
            feedback_input_new = mindspore.ops.Concat((disc_out[0], feedback_input), 1)
            ...
            feedback_input = mindspore.ops.Concat((feedback_input, gene_input[1]), 1)
            feedback_input_new = mindspore.ops.Concat((disc_out[1], feedback_input), 1)
            feedback_out = feedback_input + alpha[1] * self.norm_layer1(
                self.trans_block1(feedback_input_new))
            # feedback_out = mindspore.ops.Concat((feedback_out, gene_input[1]), 1)
            feedback_input = self.decoder_layer1(feedback_out)

            feedback_input = mindspore.ops.Concat((feedback_input, gene_input[2]), 1)
            feedback_input_new = mindspore.ops.Concat((disc_out[2], feedback_input), 1)
            feedback_out = feedback_input + alpha[2] * self.norm_layer2(
                self.trans_block2(feedback_input_new))
            # feedback_out = mindspore.ops.Concat((feedback_out, gene_input[2]), 1)
            feedback_input = self.decoder_layer2(feedback_out)

            feedback_input = mindspore.ops.Concat((feedback_input, gene_input[3]), 1)
            feedback_input_new = mindspore.ops.Concat((disc_out[3], feedback_input), 1)
            feedback_out = feedback_input + alpha[3] * self.norm_layer3(
                self.trans_block3(feedback_input_new))
            # feedback_out = mindspore.ops.Concat((feedback_out, gene_input[3]), 1)
            feedback_input = self.decoder_layer3(feedback_out)

            feedback_input = mindspore.ops.Concat((feedback_input, gene_input[4]), 1)
            feedback_input_new = mindspore.ops.Concat((disc_out[4], feedback_input), 1)
...
class UnetAFL_v5(mindspore.nn.Cell):
    """
    network structure of Feedback Adversarial Learning - cvpr 2019
    """
    def __init__(self, input_nc, output_nc, ngf=64, ndf=64, norm_layer=mindspore.nn.BatchNorm2d, use_dropout=False,
...
class panoGAN_baseline_G(mindspore.nn.Cell):
    """
    network structure of Feedback Adversarial Learning - cvpr 2019
    """
    def __init__(self, input_nc, output_nc, ngf=64, ndf=64, norm_layer=mindspore.nn.BatchNorm2d, use_dropout=False,
...
class ResnetAFLGenerator(mindspore.nn.Cell):
    """Resnet-based generator that consists of Adversarial Feedback Loop.
    """

    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=mindspore.nn.BatchNorm2d, use_dropout=False,
    ...
    def _forward_afl(self, inner_fea):
        bn1 = mindspore.nn.BatchNorm2d(self.ngf * 8).to(0)
        bn2 = mindspore.nn.BatchNorm2d(self.ngf * 4).to(0)
        bn3 = mindspore.nn.BatchNorm2d(self.ngf * 2).to(0)
        bn4 = mindspore.nn.BatchNorm2d(self.ngf * 1).to(0)
        out = inner_fea  # resBlock output
        temp_out = out + self.alpha1 * self.netGA.trans_block0(mindspore.ops.Concat([self.netGA.feedback0, out], 1))
        out = self.main.decoder_layer1(bn1(temp_out))
        temp_out = out + self.alpha2 * self.netGA.trans_block1(mindspore.ops.Concat([self.netGA.feedback1, out], 1))
        out = self.main.decoder_layer2(bn2(temp_out))
        temp_out = out + self.alpha3 * self.netGA.trans_block2(mindspore.ops.Concat([self.netGA.feedback2, out], 1))
        out = self.main.decoder_layer3(bn3(temp_out))
        temp_out = out + self.alpha4 * self.netGA.trans_block3(mindspore.ops.Concat([self.netGA.feedback3, out], 1))
    ...
    def _forward_afl_v2(self, inner_fea):
        ...
        out = self.main.decoder_layer1(
            mindspore.ops.Concat([out, self.alpha1 * self.netGA.trans_block0(mindspore.ops.Concat([self.netGA.feedback0, out], 1))], 1))
        out = self.main.decoder_layer2(
            mindspore.ops.Concat([out, self.alpha2 * self.netGA.trans_block1(mindspore.ops.Concat([self.netGA.feedback1, out], 1))], 1))
        out = self.main.decoder_layer3(
            mindspore.ops.Concat([out, self.alpha3 * self.netGA.trans_block2(mindspore.ops.Concat([self.netGA.feedback2, out], 1))], 1))
        out = self.main.decoder_layer4(
            mindspore.ops.Concat([out, self.alpha4 * self.netGA.trans_block3(mindspore.ops.Concat([self.netGA.feedback3, out], 1))], 1))
        return out

    def forward(self, x):
        gen_output = self.main(x)
        fake_AB = mindspore.ops.Concat((x, gen_output),
                            1)  # we use conditional GANs; we need to feed both input and output to the discriminator
        d_out_bk = self.netD(fake_AB)  # Fake; stop backprop to the generator by detaching fake_B
...
class ResnetAFLGenerator_FAL(mindspore.nn.Cell):
    """Resnet-based generator that consists of Adversarial Feedback Loop. with Feedback Adversarial Learning - cvpr19
    """
    ...
    def _forward_afl(self, inner_fea):
        out = inner_fea  # resBlock output
        temp_out = out + self.alpha[0] * self.netGA.trans_block1(mindspore.ops.Concat([self.netGA.feedback1, out], 1))
        out = self.main.decoder_resblock_layer(self.norm_layer1(temp_out))
        temp_out = out + self.alpha[1] * self.netGA.trans_block2(mindspore.ops.Concat([self.netGA.feedback2, out], 1))
        out = self.main.decoder_layer1(self.norm_layer2(temp_out))
        temp_out = out + self.alpha[2] * self.netGA.trans_block3(mindspore.ops.Concat([self.netGA.feedback3, out], 1))
        out = self.main.decoder_layer2(self.norm_layer3(temp_out))
        temp_out = out + self.alpha[3] * self.netGA.trans_block4(mindspore.ops.Concat([self.netGA.feedback4, out], 1))
        out = self.main.decoder_layer3(self.norm_layer4(temp_out))
        temp_out = out + self.alpha[4] * self.netGA.trans_block5(mindspore.ops.Concat([self.netGA.feedback5, out], 1))
...
class MainNet(mindspore.nn.Cell):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=mindspore.nn.BatchNorm2d, use_dropout=False, n_blocks=6,
                 padding_type='reflect'):
        assert (n_blocks >= 0)
        ...
        self.encoder_layer1 = mindspore.nn.SequentialCell(
            # state size: 64 channel
            mindspore.nn.Conv2d(input_nc, ngf, kernel_size=4,
        ...
        self.encoder_layer2 = mindspore.nn.SequentialCell(
            ...
            mindspore.nn.Conv2d(ngf, ngf * 2, kernel_size=4,
                      stride=2, padding=1, has_bias=use_bias, pad_mode='pad'),
            norm_layer(ngf),
            mindspore.nn.ReLU()
        )
        self.encoder_layer3 = mindspore.nn.SequentialCell(
            # state size: 256 channel
            mindspore.nn.Conv2d(ngf * 2, ngf * 4, kernel_size=4,
                      stride=2, padding=1, has_bias=use_bias, pad_mode='pad'),
            norm_layer(ngf),
            mindspore.nn.ReLU()
        )
        self.encoder_layer4 = mindspore.nn.SequentialCell(
            ...
            mindspore.nn.Conv2d(ngf * 4, ngf * 8, kernel_size=4,
                      stride=2, padding=1, has_bias=use_bias, pad_mode='pad'),
            norm_layer(ngf),
            mindspore.nn.ReLU()
        )
        ...
        for i in range(n_blocks):  # add ResNet blocks
            model += [ResnetBlock(ngf * 8, padding_type=padding_type,
                                  norm_layer=norm_layer, use_dropout=use_dropout, has_bias=use_bias, pad_mode='pad')]
        self.resnetBlock = mindspore.nn.SequentialCell(*model)
        """
        self.decoder_layer1 = mindspore.nn.SequentialCell(
            # state size: 256 channel
            mindspore.nn.Conv2d(ngf * 8, ngf * 8,
        ...
        self.decoder_layer1 = mindspore.nn.SequentialCell(
            # state size: 256 channel
            mindspore.nn.Conv2dTranspose(ngf * 8, ngf * 4,
        ...
        self.decoder_layer2 = mindspore.nn.SequentialCell(
            # state size: 128
            mindspore.nn.Conv2dTranspose(ngf * 4, ngf * 2,
                               kernel_size=4, stride=2,
                               padding=1, output_padding=0,
                               has_bias=use_bias, pad_mode='pad'),
            norm_layer(ngf * 2),
        ...
        self.decoder_layer3 = mindspore.nn.SequentialCell(
            # state size: 64 channel
            mindspore.nn.Conv2dTranspose(ngf * 2, ngf * 1,
                               kernel_size=4, stride=2,
                               padding=1, output_padding=0,
                               has_bias=use_bias, pad_mode='pad'),
            norm_layer(ngf * 1),
        ...
        self.decoder_layer4 = mindspore.nn.SequentialCell(
            ...
            mindspore.nn.Conv2dTranspose(ngf, output_nc,
                               kernel_size=4, stride=2,
                               padding=1, output_padding=0,
                               has_bias=use_bias, pad_mode='pad'),
            mindspore.nn.Tanh()
...
class TransBlockDual(mindspore.nn.Cell):
    def __init__(self, afl_type=1, input_nc=6, output_nc=3):
        super(TransBlockDual, self).__init__()
        if afl_type == 1:
            self.main = mindspore.nn.SequentialCell(
                mindspore.nn.Conv2d(input_nc, output_nc, kernel_size=3, stride=1, padding=1),
                mindspore.nn.InstanceNorm2d(output_nc),
                mindspore.nn.ReLU(True),
                mindspore.nn.Conv2d(output_nc, output_nc, kernel_size=3, stride=1, padding=1),
                mindspore.nn.InstanceNorm2d(output_nc),
                mindspore.nn.ReLU(True))
        elif afl_type == 2:
            pass
...
class Parm:
    def __init__(self, ngf=64, norm_layer=mindspore.nn.BatchNorm2d, use_dropout=False, afl_type=1, padding_type='reflect'):
        self.size = ngf
...
class GeneratorAFL(mindspore.nn.Cell):
...
class ResnetBlock(mindspore.nn.Cell):
    ...
    def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias):
        ...
        if padding_type == 'reflect':
            conv_block += [mindspore.nn.Pad(paddings=((1, 1), (1, 1)), mode="REFLECT")]
        elif padding_type == 'replicate':
            conv_block += [mindspore.nn.Pad(paddings=((1, 1), (1, 1)), mode="SYMMETRIC")]
        elif padding_type == 'zero':
            p = 1
        else:
            raise NotImplementedError('padding [%s] is not implemented' % padding_type)

        conv_block += [mindspore.nn.Conv2d(dim, dim, kernel_size=3, padding=p, has_bias=use_bias, pad_mode='pad'), norm_layer(dim), mindspore.nn.ReLU(True)]
        if use_dropout:
            conv_block += [mindspore.nn.Dropout(0.5)]

        p = 0
        if padding_type == 'reflect':
            conv_block += [mindspore.nn.Pad(1)]
        elif padding_type == 'replicate':
            conv_block += [mindspore.nn.Pad(1)]
        elif padding_type == 'zero':
            p = 1
            ...
            raise NotImplementedError('padding [%s] is not implemented' % padding_type)
        conv_block += [mindspore.nn.Conv2d(dim, dim, kernel_size=3, padding=p, has_bias=use_bias, pad_mode='pad'), norm_layer(dim)]

        return mindspore.nn.SequentialCell(*conv_block)
...
class UnetGenerator(mindspore.nn.Cell):
    """Create a Unet-based generator"""

    def __init__(self, input_nc, output_nc, num_downs, ngf=64, norm_layer=mindspore.nn.BatchNorm2d, use_dropout=False):
...
class UnetSkipConnectionBlock(mindspore.nn.Cell):
    ...
    def __init__(self, outer_nc, inner_nc, input_nc=None,
                 submodule=None, outermost=False, innermost=False, norm_layer=mindspore.nn.BatchNorm2d, use_dropout=False):
        ...
        if input_nc is None:
            input_nc = outer_nc
        downconv = mindspore.nn.Conv2d(input_nc, inner_nc, kernel_size=4,
                             stride=2, padding=1, has_bias=use_bias, pad_mode='pad')
        downrelu = mindspore.nn.LeakyReLU(0.2, True)
        downnorm = norm_layer(inner_nc)
        uprelu = mindspore.nn.ReLU(True)
        upnorm = norm_layer(outer_nc)

        if outermost:
            upconv = mindspore.nn.Conv2dTranspose(inner_nc * 2, outer_nc,
                                        kernel_size=4, stride=2,
                                        padding=1)
            down = [downconv]
            up = [uprelu, upconv, mindspore.nn.Tanh()]
            model = down + [submodule] + up
        elif innermost:
            upconv = mindspore.nn.Conv2dTranspose(inner_nc, outer_nc,
                                        kernel_size=4, stride=2,
                                        padding=1, has_bias=use_bias, pad_mode='pad')
            ...
            if use_dropout:
                model = down + [submodule] + up + [mindspore.nn.Dropout(0.5)]
            else:
                model = down + [submodule] + up
    ...
    def forward(self, x):
        if self.outermost:
            ...
            return mindspore.ops.Concat([x, self.model(x)], 1)


class Unet6cGenerator(mindspore.nn.Cell):
...
class UnetAFL_4a(mindspore.nn.Cell):
    """Resnet-based generator that consists of Adversarial Feedback Loop. with Feedback Adversarial Learning - cvpr19
    """
    ...
    def _forward_afl(self, fea_list):
        ...
        input_fea = input_fea + self.alpha[0] * self.norm_layer0(
            self.netGA.trans_block0(mindspore.ops.Concat([self.netGA.feedback0, input_fea], 1)))
        # input_fea = mindspore.ops.Concat((input_fea, fea_list[0]), 1)
        input_fea = self.main.up4(input_fea, fea_list[1])

        input_fea = input_fea + self.alpha[1] * self.norm_layer1(
            self.netGA.trans_block1(mindspore.ops.Concat([self.netGA.feedback1, input_fea], 1)))
        # input_fea = mindspore.ops.Concat((input_fea, fea_list[1]), 1)
        input_fea = self.main.up5(input_fea, fea_list[2])

        input_fea = input_fea + self.alpha[2] * self.norm_layer2(
            self.netGA.trans_block2(mindspore.ops.Concat([self.netGA.feedback2, input_fea], 1)))
        # input_fea = mindspore.ops.Concat((input_fea, fea_list[2]), 1)
        input_fea = self.main.up6(input_fea, fea_list[3])

        input_fea = input_fea + self.alpha[3] * self.norm_layer3(
            self.netGA.trans_block3(mindspore.ops.Concat([self.netGA.feedback3, input_fea], 1)))
        # input_fea = mindspore.ops.Concat((input_fea, fea_list[3]), 1)
        input_fea = self.main.up7(input_fea, fea_list[4])

        input_fea = input_fea + self.alpha[4] * self.norm_layer4(
            self.netGA.trans_block4(mindspore.ops.Concat([self.netGA.feedback4, input_fea], 1)))
        # input_fea = mindspore.ops.Concat((input_fea, fea_list[4]), 1)
...
class UNetDown(mindspore.nn.Cell):
    def __init__(self, in_size, out_size, normalize=True, dropout=0.0):
        super(UNetDown, self).__init__()
        layers = [mindspore.nn.Conv2d(in_size, out_size, 4, 2, 1, bias=True)]
        if normalize:
            layers.append(mindspore.nn.InstanceNorm2d(out_size))
        layers.append(mindspore.nn.LeakyReLU(0.2))
        if dropout:
            layers.append(mindspore.nn.Dropout(dropout))
        self.model = mindspore.nn.SequentialCell(*layers)

    def forward(self, x):
        return self.model(x)


class UNetUp(mindspore.nn.Cell):
    def __init__(self, in_size, out_size, dropout=0.0):
        super(UNetUp, self).__init__()
        layers = [
            mindspore.nn.Conv2dTranspose(in_size, out_size, 4, 2, 1, bias=True),
            mindspore.nn.InstanceNorm2d(out_size),
            mindspore.nn.ReLU(),
        ]
        if dropout:
            layers.append(mindspore.nn.Dropout(dropout))
    ...
    def forward(self, x, skip_input):
        x = self.model(x)
        x = mindspore.ops.Concat((x, skip_input), 1)

        return x


class BasicUNetGenerator(mindspore.nn.Cell):
    ...
    def __init__(self, input_nc=3, output_nc=3):
        ...
        self.final = mindspore.nn.SequentialCell(
            mindspore.nn.ResizeBilinear(scale_factor=2),
            nn.ZeroPad2d((1, 0, 1, 0)),
            mindspore.nn.Conv2d(128, output_nc, 4, padding=1),
            mindspore.nn.Tanh(),
        )
...
class NLayerDiscriminator(mindspore.nn.Cell):
    """Defines a PatchGAN discriminator"""

    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=mindspore.nn.BatchNorm2d):
        ...
        kw = 4
        padw = 1
        sequence = [mindspore.nn.Conv2d(input_nc, ndf, kernel_size=kw, stride=2, padding=padw), mindspore.nn.LeakyReLU(0.2, True)]
        nf_mult = 1
        nf_mult_prev = 1
        for n in range(1, n_layers):  # gradually increase the number of filters
            ...
            nf_mult = min(2 ** n, 8)
            sequence += [
                mindspore.nn.Conv2d(ndf * nf_mult_prev, ndf * nf_mult, kernel_size=kw, stride=2, padding=padw, has_bias=use_bias, pad_mode='pad'),
                norm_layer(ndf * nf_mult),
                mindspore.nn.LeakyReLU(0.2, True)
            ]

        nf_mult_prev = nf_mult
        nf_mult = min(2 ** n_layers, 8)
        sequence += [
            mindspore.nn.Conv2d(ndf * nf_mult_prev, ndf * nf_mult, kernel_size=kw, stride=1, padding=padw, has_bias=use_bias, pad_mode='pad'),
            norm_layer(ndf * nf_mult),
            mindspore.nn.LeakyReLU(0.2, True)
        ]

        sequence += [
            mindspore.nn.Conv2d(ndf * nf_mult, 1, kernel_size=kw, stride=1, padding=padw)]  # output 1 channel prediction map
        self.model = mindspore.nn.SequentialCell(*sequence)
...
class PixelDiscriminator(mindspore.nn.Cell):
    """Defines a 1x1 PatchGAN discriminator (pixelGAN)"""

    def __init__(self, input_nc, ndf=64, norm_layer=mindspore.nn.BatchNorm2d):
        ...
        self.net = [
            mindspore.nn.Conv2d(input_nc, ndf, kernel_size=1, stride=1, padding=0),
            mindspore.nn.LeakyReLU(0.2, True),
            mindspore.nn.Conv2d(ndf, ndf * 2, kernel_size=1, stride=1, padding=0, has_bias=use_bias, pad_mode='pad'),
            norm_layer(ndf * 2),
            mindspore.nn.LeakyReLU(0.2, True),
            mindspore.nn.Conv2d(ndf * 2, 1, kernel_size=1, stride=1, padding=0, has_bias=use_bias, pad_mode='pad')]

        self.net = mindspore.nn.SequentialCell(*self.net)
...
class Discriminator_UNet(mindspore.nn.Cell):
    """Defines a PatchGAN discriminator"""
    ...
    def __init__(self, input_nc, ndf=64, norm_layer=mindspore.nn.BatchNorm2d):
        ...
        self.l1 = mindspore.nn.SequentialCell(  # input size is 256
            mindspore.nn.Conv2d(input_nc, ndf, kernel_size=4, stride=2, padding=1, has_bias=use_bias, pad_mode='pad'),
            mindspore.nn.LeakyReLU(0.2, inplace=True))
        self.l2 = mindspore.nn.SequentialCell(  # input size is 128
            mindspore.nn.Conv2d(ndf, ndf * 2, kernel_size=4, stride=2, padding=1, has_bias=use_bias, pad_mode='pad'),
            mindspore.nn.LeakyReLU(0.2, inplace=True))
        self.l3 = mindspore.nn.SequentialCell(  # input size is 64
            mindspore.nn.Conv2d(ndf * 2, ndf * 4, kernel_size=4, stride=2, padding=1, has_bias=use_bias, pad_mode='pad'),
            mindspore.nn.LeakyReLU(0.2, inplace=True))
        self.l4 = mindspore.nn.SequentialCell(  # input size is 32
            mindspore.nn.Conv2d(ndf * 4, ndf * 8, kernel_size=4, stride=2, padding=1, has_bias=use_bias, pad_mode='pad'),
            mindspore.nn.LeakyReLU(0.2, inplace=True))
        self.l5 = mindspore.nn.SequentialCell(  # input size is 16
            mindspore.nn.Conv2d(ndf * 8, ndf * 8, kernel_size=4, stride=2, padding=1, has_bias=use_bias, pad_mode='pad'),
            mindspore.nn.LeakyReLU(0.2, inplace=True))
        self.l6 = mindspore.nn.SequentialCell(  # input size is 15
            mindspore.nn.Conv2d(ndf * 8, 1, kernel_size=4, stride=1, padding=1)
        )
...
class AFLDiscriminator_FAL(mindspore.nn.Cell):
    ...
    def __init__(self, input_nc, ndf=64, norm_layer=mindspore.nn.BatchNorm2d):
        ...
        ndf_out = ndf
        self.l1 = mindspore.nn.SequentialCell(  # input size is 256
            mindspore.nn.Conv2d(ndf_in, ndf_out, kernel_size=3, stride=1, padding=1, has_bias=use_bias, pad_mode='pad'),
            mindspore.nn.AvgPool2d(kernel_size=3, stride=2),
            norm_layer(ndf_out),
            mindspore.nn.LeakyReLU(0.2, inplace=True))
        ...
        ndf_in = ndf_out
        ndf_out = ndf_in * 2
        self.l2 = mindspore.nn.SequentialCell(  # input size is 256
            mindspore.nn.Conv2d(ndf_in, ndf_out, kernel_size=3, stride=1, padding=1, has_bias=use_bias, pad_mode='pad'),
            mindspore.nn.AvgPool2d(kernel_size=3, stride=2),
            norm_layer(ndf_out),
            mindspore.nn.LeakyReLU(0.2, inplace=True))
        ...
        ndf_in = ndf_out
        ndf_out = ndf_in * 2
        self.l3 = mindspore.nn.SequentialCell(  # input size is 256
            mindspore.nn.Conv2d(ndf_in, ndf_out, kernel_size=3, stride=1, padding=1, has_bias=use_bias, pad_mode='pad'),
            mindspore.nn.AvgPool2d(kernel_size=3, stride=2),
            norm_layer(ndf_out),
            mindspore.nn.LeakyReLU(0.2, inplace=True))
        ...
        ndf_in = ndf_out
        ndf_out = ndf_in * 2
        self.l4 = mindspore.nn.SequentialCell(  # input size is 256
            mindspore.nn.Conv2d(ndf_in, ndf_out, kernel_size=3, stride=1, padding=1, has_bias=use_bias, pad_mode='pad'),
            mindspore.nn.AvgPool2d(kernel_size=3, stride=2),
            norm_layer(ndf_out),
            mindspore.nn.LeakyReLU(0.2, inplace=True))
        ...
        ndf_in = ndf_out
        ndf_out = ndf_in
        self.l5 = mindspore.nn.SequentialCell(  # input size is 256
            mindspore.nn.Conv2d(ndf_in, ndf_out, kernel_size=3, stride=1, padding=1, has_bias=use_bias, pad_mode='pad'),
            # mindspore.nn.AvgPool2d(kernel_size=3, stride=2, padding=1, ceil_mode=True),
            norm_layer(ndf_out),
            mindspore.nn.LeakyReLU(0.2, inplace=True))
        ...
        self.l6 = mindspore.nn.SequentialCell(  # input size is 15
            # mindspore.nn.ReLU(True),
            mindspore.nn.Conv2d(ndf_in, ndf_out, kernel_size=3, stride=1, padding=0, has_bias=use_bias, pad_mode='pad')
        )
...
class Discriminator_PatchGAN_Feedback(mindspore.nn.Cell):
    """the discriminator is borrowed from Feedback Adversarial Learning -cvpr19"""
    def __init__(self, input_nc, ndf=64, norm_layer=mindspore.nn.BatchNorm2d):
...
class FeaturePyramidDiscriminator(mindspore.nn.Cell):
    """the discriminator is borrowed from Feedback Adversarial Learning -cvpr19"""
    def __init__(self, input_nc, ndf=64, norm_layer=mindspore.nn.BatchNorm2d, disc_nc_scale=[1,1,1,1,1]):
        ...
        #-------- top-down pathway:
        ndf_out = ndf * 2
        self.lat1 = mindspore.nn.SequentialCell(
            mindspore.nn.Conv2d(ndf, ndf_out, kernel_size=1),
            norm_layer(ndf_out),
            mindspore.nn.LeakyReLU(0.2, True))
        self.lat2 = mindspore.nn.SequentialCell(
            mindspore.nn.Conv2d(ndf * 2, ndf_out, kernel_size=1),
            norm_layer(ndf_out),
            mindspore.nn.LeakyReLU(0.2, True))
        self.lat3 = mindspore.nn.SequentialCell(
            mindspore.nn.Conv2d(ndf * 4, ndf_out, kernel_size=1),
            norm_layer(ndf_out),
            mindspore.nn.LeakyReLU(0.2, True))
        self.lat4 = mindspore.nn.SequentialCell(
            mindspore.nn.Conv2d(ndf * 8, ndf_out, kernel_size=1),
            norm_layer(ndf_out),
            mindspore.nn.LeakyReLU(0.2, True))
        self.lat5 = mindspore.nn.SequentialCell(
            mindspore.nn.Conv2d(ndf * 8, ndf_out, kernel_size=1),
            norm_layer(ndf_out),
            mindspore.nn.LeakyReLU(0.2, True))

        # upsampling
        self.up = mindspore.nn.ResizeBilinear(scale_factor=2, mode='bilinear')

        # final layers
        ndf_in = ndf_out
        ndf_out = ndf * disc_nc_scale[0]
        self.final1 = mindspore.nn.SequentialCell(
            mindspore.nn.Conv2d(ndf_in, ndf_out, kernel_size=1),
            norm_layer(ndf_out),
            mindspore.nn.LeakyReLU(0.2, True))
        ndf_out = ndf * disc_nc_scale[1]
        self.final2 = mindspore.nn.SequentialCell(
            mindspore.nn.Conv2d(ndf_in, ndf_out, kernel_size=1),
            norm_layer(ndf_out),
            mindspore.nn.LeakyReLU(0.2, True))
        ndf_out = ndf * disc_nc_scale[2]
        self.final3 = mindspore.nn.SequentialCell(
            mindspore.nn.Conv2d(ndf_in, ndf_out, kernel_size=1),
            norm_layer(ndf_out),
            mindspore.nn.LeakyReLU(0.2, True))
        ndf_out = ndf * disc_nc_scale[3]
        self.final4 = mindspore.nn.SequentialCell(
            mindspore.nn.Conv2d(ndf_in, ndf_out, kernel_size=1),
            norm_layer(ndf_out),
            mindspore.nn.LeakyReLU(0.2, True))
        ndf_out = ndf * disc_nc_scale[4]
        self.final5 = mindspore.nn.SequentialCell(
            mindspore.nn.Conv2d(ndf_in, ndf_out, kernel_size=1),
            norm_layer(ndf_out),
            mindspore.nn.LeakyReLU(0.2, True))
        ...
        ndf_in = ndf_out
        ndf_out = ndf_in
        self.tf = mindspore.nn.Conv2d(ndf_in, 1, kernel_size=1)
        self.seg = mindspore.nn.Conv2d(ndf_in, ndf_out, kernel_size=1)
