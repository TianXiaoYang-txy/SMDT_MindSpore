...
class PanoAlignedDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        A = v_transforms.ToTensor(A_tmp)
        for i in range(1, 4):
            #A.append(A_tmp.rotate(90*i))
            A = x2ms_adapter.cat((A, v_transforms.ToTensor(A_tmp.rotate(90*i))),2)

        #A = transforms.ToTensor()(A)
        B = v_transforms.ToTensor(B)
        D = v_transforms.ToTensor(D)
        ...
        A = v_transforms.Normalize(A)
        B = v_transforms.Normalize(B)
        D = v_transforms.Normalize(D)
