...
def get_transform(opt, params=None, grayscale=False, method=Image.BICUBIC, convert=True):
    ...
    if convert:
        ...
        if grayscale:
            transform_list += [v_transforms.Normalize((0.5,), (0.5,))]
        else:
            transform_list += [v_transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))]
...
def get_transform_pano(opt, params=None, grayscale=False, method=Image.BICUBIC, convert=True):
    ...
    if convert:
        ...
        if grayscale:
            transform_list.append(v_transforms.Normalize((0.5,), (0.5,)))
        else:
...
def __flip(img, flip):
    if flip:
        return x2ms_adapter.tensor_api.transpose(img, Image.FLIP_LEFT_RIGHT)
    return img
